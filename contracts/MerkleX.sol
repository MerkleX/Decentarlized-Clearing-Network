...
contract MerkleX {
  ...
  Session[2**24] sessions;

  uint256 next_session_pointer;
  ...
  function create_session(address trade_address) public payable {
    assembly {
      // Last valid pointer: (2^24 - 1) * 32 = 536870880
      let current_session_pointer := sload(next_session_pointer_slot)
      if gt(current_session_pointer, 536870880) {
        stop()
      }
      
      // Does caller have required fee
      let session_fee := sload(add(current_settings_slot, 32))
      if not(eq(callvalue, session_fee)) {
        stop()
      }

      // Store new session data
      sstore(add(sessions_slot, current_session_pointer), address)
      sstore(add(sessions_slot, add(current_session_pointer, 32)), trade_address)
      sstore(next_session_pointer_slot, add(current_session_pointer, 64))
      
      // Collect fee
      if gt(session_fee, 0) {
          sstore(fee_balance_slot, add(sload(fee_balance_slot), session_fee))
      }
    }
  }
