...
contract DCNMemoryConflict is DCN {
  function test_asset_exchange_no_conflict(uint32 asset_id, uint64 exchange_id) {
    assembly {
  ...
  function test_asset_user_no_conflict(uint32 asset_id, uint64 user_id) {
    assembly {
  ...
  function test_user_exchange_no_conflict(uint64 user_id, uint64 exchange_id) {
    assembly {
      let user_ptr := USER_PTR_(user_id)
      let exchange_ptr := EXCHANGE_PTR_(exchange_id)
  ...
  function test_exchange_balance_in_exchange(uint64 exchange_id, uint32 asset_id) {
    assembly {
      let exchange_ptr := EXCHANGE_PTR_(exchange_id)
      let balance_ptr := EXCHANGE_BALANCE_PTR_(exchange_ptr, asset_id)

      CONTAINS(Exchange, uint256, exchange_ptr, balance_ptr)
    }
  }

  function test_user_balance_in_user(uint64 user_id, uint32 asset_id) {
    assembly {
      let user_ptr := USER_PTR_(user_id)
      let balance_ptr := USER_BALANCE_PTR_(user_ptr, asset_id)

      CONTAINS(User, uint256, user_ptr, balance_ptr)
    }
  }

  function test_user_session_in_user(uint64 user_id, uint64 exchange_id) {
    assembly {
      let user_ptr := USER_PTR_(user_id)
  ...
  function test_user_session_balance_inside(uint64 user_id, uint64 exchange_id, uint32 asset_id) {
    assembly {
      let user_ptr := USER_PTR_(user_id)
      let session_ptr := SESSION_PTR_(user_ptr, exchange_id)
  ...
  function test_market_state_in_session(uint64 user_id, uint64 exchange_id, uint32 quote_asset_id, uint32 base_asset_id) {
    assembly {
      let user_ptr := USER_PTR_(user_id)
      let session_ptr := SESSION_PTR_(user_ptr, exchange_id)
  ...
  function test_market_state_session_balance_no_conflict(uint64 user_id, uint64 exchange_id,
                                                         uint32 asset_id, uint32 quote_asset_id, uint32 base_asset_id) {
    assembly {
      let user_ptr := USER_PTR_(user_id)
      let session_ptr := SESSION_PTR_(user_ptr, exchange_id)
