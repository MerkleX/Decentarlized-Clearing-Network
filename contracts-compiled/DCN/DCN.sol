...
//  )
  ...
  constructor() public {
      assembly {
          ...
          sstore(assets_slot, or(mul(/* symbol */ /* "ETH " */ 0x45544820, 0x100000000000000000000000000000000000000000000000000000000), mul(/* unit_scale */ 100000000, 0x10000000000000000000000000000000000000000)))
      }
  }
  ...
  function close_session(uint32 session_id) public {
    ...
    assembly {
      ...
      ether_balance := add(ether_balance, mul(/* ETHER(session_ether_data).balance */ and(div(session_ether_data, 0x1), 0xffffffffffffffff), 100000000))
